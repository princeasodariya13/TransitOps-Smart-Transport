...
const FuelModal = ({ isOpen, onClose, onRefresh, fuelEntry = null }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (fuelEntry) {
                setFormData({
                    vehicleId: fuelEntry.vehicle?.id || fuelEntry.vehicleId?.id || fuelEntry.vehicleId || '',
                    tripId: fuelEntry.trip?.id || fuelEntry.tripId?.id || fuelEntry.tripId || '',
    ...
    const fetchResources = async () => {
        try {
            ...
            setVehicles(vRes.data.data.filter(v => v.status !== 'RETIRED' || (isEdit && v.id === formData.vehicleId)));
            // Only show recent dispatched/completed trips
            setTrips(tRes.data.data.filter(t => ['DISPATCHED', 'COMPLETED'].includes(t.status) || (isEdit && t.id === formData.tripId)));
        } catch (err) {
    ...
    const filteredTrips = formData.vehicleId
        ? trips.filter(t => (t.vehicleId?.id || t.vehicleId) === formData.vehicleId)
        : trips;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                await api.put(`/fuel/${fuelEntry.id}`, payload);
                            ...
                            >
                                ...
                                {filteredTrips.map(t => (
                                    <option key={t.id} value={t.id}>
                                        {t.uid || t.id?.slice(-6) || 'TRP'} - {t.vehicle?.name || 'Vehicle'} — {t.status} ({new Date(t.createdAt).toLocaleDateString()})
                                    </option>
                                ))}
                            </select>
                        </div>
