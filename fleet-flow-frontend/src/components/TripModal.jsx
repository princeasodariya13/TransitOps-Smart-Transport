...
const TripModal = ({ isOpen, onClose, onRefresh, trip = null }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (trip) {
                setFormData({
                    vehicleId: trip.vehicle?.id || trip.vehicleId?.id || trip.vehicleId || '',
                    driverId: trip.driver?.id || trip.driverId?.id || trip.driverId || '',
    ...
    const fetchResources = async () => {
        try {
            ...
            setVehicles(vRes.data.data.filter(v =>
                v.status === 'AVAILABLE' || (isEdit && v.id === formData.vehicleId)
            ));
            setDrivers(dRes.data.data.filter(d =>
                d.status === 'ON_DUTY' || (isEdit && d.id === formData.driverId)
            ));
        } catch (err) {
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                                ...
                                >
                                    ...
                                    {vehicles.map(v => (
                                        <option key={v.id} value={v.id}>
                                            {v.uid || v.id.slice(-6)} - {v.name} ({v.licensePlate})
                                        </option>
                                    ))}
                                </select>
                            </div>
                                ...
                                >
                                    ...
                                    {drivers.map(d => (
                                        <option key={d.id} value={d.id}>
                                            {d.uid || d.id.slice(-6)} - {d.name} ({d.category})
                                        </option>
                                    ))}
                                </select>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <div className="flex justify-between">
                                ...
                                {formData.vehicleId && (
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">
                                        Max: {vehicles.find(v => v.id === formData.vehicleId)?.maxCapacity || '0'}kg
                                    </span>
                                )}
                            </div>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className={`w-full pl-10 pr-4 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-primary-500 transition-all font-medium ${formData.cargoWeight && vehicles.find(v => v.id === formData.vehicleId)?.maxCapacity < Number(formData.cargoWeight)
                            ...
                            {formData.cargoWeight && vehicles.find(v => v.id === formData.vehicleId)?.maxCapacity < Number(formData.cargoWeight) && (
                    ...
                    <div className="pt-4 flex space-x-3">
                        ...
                        <button
                            ...
                            disabled={
                                ...
                                !formData.vehicleId ||
                                !formData.driverId ||
                                (Number(formData.cargoWeight) > (vehicles.find(v => v.id === formData.vehicleId)?.maxCapacity || 0))
                            }
