...
const MaintenanceModal = ({ isOpen, onClose, onRefresh, maintenance = null }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (maintenance) {
                setFormData({
                    vehicleId: maintenance.vehicle?.id || maintenance.vehicleId?.id || maintenance.vehicleId || '',
    ...
    const fetchVehicles = async () => {
        try {
            ...
            setVehicles(res.data.filter(v =>
                (v.status !== 'IN_SHOP' && v.status !== 'RETIRED') ||
                (isEdit && (v.id === formData.vehicleId))
            ));
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await api.put(`/maintenances/${maintenance.id}`, formData);
