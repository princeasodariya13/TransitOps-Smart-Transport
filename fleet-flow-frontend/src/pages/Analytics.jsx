...
import {
    ...
    TrendingUp, Activity, AlertTriangle, CheckCircle2, Download, FileText
} from 'lucide-react';
...
const Analytics = () => {
        ...
        const fetchAll = async () => {
            try {
                ...
                setData({
                    ...
                    trips: tRes.data.data,
                    maintenances: mRes.data.data,
                    fuel: fRes.data.data,
                });
            } catch (err) {
                console.error(err);
    ...
    const topVehicles = vehicles
        .map(v => {
            const vTrips = completedTrips.filter(t => (t.vehicleId?.id || t.vehicleId) === (v.id));
            const vFuel = fuel.filter(f => (f.vehicleId?.id || f.vehicleId) === (v.id));
            const vMaint = maintenances.filter(m => (m.vehicleId?.id || m.vehicleId) === (v.id));
    ...
    const handleExportCSV = async () => {
        try {
            const response = await api.get('/analytics/vehicles/export', { responseType: 'blob' });
    ...
    const handleExportPDF = () => {
        ...
        autoTable(doc, {
            ...
            body: [
                ...
                ['Active Vehicles (On Trip)', onTripVehicles.toString()],
                ['Vehicles In Shop', inShopVehicles.toString()],
                ['Total Revenue', `Rs ${totalRevenue.toLocaleString()}`],
                ['Total Fuel Cost', `Rs ${totalFuelCost.toLocaleString()}`],
                ['Total Maintenance Cost', `Rs ${totalMaintenanceCost.toLocaleString()}`],
        ...
        // Vehicle Performance
        doc.text('Vehicle ROI Leaderboard', 14, doc.lastAutoTable.finalY + 15);
        
        autoTable(doc, {
            startY: doc.lastAutoTable.finalY + 20,
            head: [['Vehicle', 'Revenue', 'Operating Cost', 'ROI %']],
            body: topVehicles.map(v => [
                v.name,
        ...
        doc.save('fleetflow-analytics-report.pdf');
    };
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="flex items-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50 transition-colors font-medium text-sm"
                    >
                        <FileText size={16} />
                        Export PDF
                    </button>
