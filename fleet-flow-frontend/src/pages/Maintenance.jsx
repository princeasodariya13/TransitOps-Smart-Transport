...
const Maintenance = () => {
    ...
    const fetchLogs = async () => {
        setLoading(true);
        try {
            const { data: res } = await api.get('/maintenances');
            setLogs(res.data);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Service Logs</h1>
                    <p className="text-slate-500 text-sm">Track vehicle repairs, maintenance, and operational costs</p>
                </div>
                <button
