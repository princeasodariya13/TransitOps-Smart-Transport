                            ...
                            <>
                                ...
                                {changeableStatuses.map(s => {
                                    ...
                                    return (
                                        <button
                                            key={s}
                                            onClick={() => { onStatusChange(driver.id, s); setOpen(false); }}
...
const Drivers = () => {
    ...
    const fetchDrivers = async () => {
        setLoading(true);
        try {
            const { data: res } = await api.get('/drivers');
            setDrivers(res.data);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Driver Workforce</h1>
                    <p className="text-slate-500 text-sm">Manage your qualified personnel and assignments</p>
                </div>
                <button
                ...
                <div className="p-3 sm:p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full pl-9 text-gray-700 pr-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary-500"
                            value={searchTerm}
