...
const FuelLogs = () => {
    ...
    const fetchLogs = async () => {
        setLoading(true);
        try {
            const { data: res } = await api.get('/fuel');
            setLogs(res.data);
        } catch (err) {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await api.delete(`/fuel/${deletingEntry.id}`);
    ...
    const filteredLogs = logs.filter(log => {
        ...
        const matchesVehicle = filterVehicle === 'ALL' || log.vehicle?.id === filterVehicle;
        return matchesSearch && matchesVehicle;
    });

    const uniqueVehicles = Array.from(new Map(logs.map(log => [log.vehicle?.id, log.vehicle])).values()).filter(Boolean);
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Fuel &amp; Expenses</h1>
                    <p className="text-slate-500 text-sm">Track fleet fuel consumption and operating costs</p>
                </div>
                <button
