            ...
            <AnimatePresence>
                {open && (
                    ...
                    >
                        {trip.status === 'DRAFT' && (
                            <button
                                onClick={() => { onDispatch(trip.id); setOpen(false); }}
...
const Trips = () => {
    ...
    const filteredTrips = trips.filter(t => {
        const matchesSearch =
            (t.vehicle?.name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Trip Dispatch Center</h1>
                    <p className="text-slate-500 text-sm">Coordinate and track active vehicle assignments</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-3 sm:p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Search by vehicle, operator, or ID..."
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[720px]">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="7" className="px-6 py-10 text-center text-slate-500 italic">Synchronizing dispatch data...</td></tr>
                            ) : filteredTrips.length === 0 ? (
                                <tr><td colSpan="7" className="px-6 py-10 text-center text-slate-500 italic">No scheduled trips match your search/filters</td></tr>
                            ) : filteredTrips.map((t) => {
